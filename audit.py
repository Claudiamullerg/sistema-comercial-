...
def _norm_tag(t: str) -> str:
    """Normaliza un nombre de etiqueta ignorando el sufijo de fecha/hora que ManyChat
    añade al duplicar flujos: «link_clicked (2026-05-25 14:15:19)» → «linkclicked»."""
    t = re.sub(r"\(\s*\d{4}-\d{2}-\d{2}[^)]*\)", " ", t or "")
    return _norm(t)
...
def clasificar_etiquetas(db: Session) -> dict:
    ...
    for c in _activos(db):
        for t in (c.mc_tags or []):
            if isinstance(t, dict) and t.get("name"):
                uso[t["name"]] += 1
    # ¿Tenemos datos suficientes para afirmar que una etiqueta no se usa?
    # Solo si ya hay contactos sincronizados desde ManyChat. Si no, "0 contactos"
    # significa "todavía no lo sabemos", no "nadie la tiene".
    hay_datos = any((c.mc_tags or c.sync_status == "OK") for c in _activos(db))
    por_norm: dict[str, list[McTag]] = defaultdict(list)
    for t in tags:
        t.contact_count = uso.get(t.name, t.contact_count or 0)
        por_norm[_norm_tag(t.name)].append(t)
    resumen = defaultdict(int)
    hoy = datetime.utcnow()
    for t in tags:
        clase, razon = "ACTIVA", None
        hermanas = por_norm[_norm_tag(t.name)]
        if len(hermanas) > 1:
            otras = ", ".join(f"«{o.name}»" for o in hermanas if o is not t)
            clase, razon = "DUPLICADA", f"Mismo nombre que {otras} (ManyChat añade la fecha al duplicar un flujo)"
        elif not hay_datos:
            clase, razon = "SIN_DATOS", "Todavía no hay contactos sincronizados: no se puede saber si se usa"
    ...
    for c in _activos(db):
        nombres = {_norm_tag(t.get("name", "")) for t in (c.mc_tags or []) if isinstance(t, dict)}
        for a, b in palabras:
...
def proponer_etiquetas(db: Session) -> int:
    n = 0
    # Nunca se propone retirar una etiqueta si no hay datos para afirmarlo (SIN_DATOS).
    retirables = {t.name: t for t in db.scalars(select(McTag).where(McTag.classification.in_(["SIN_USO", "DUPLICADA", "OBSOLETA"])))}
    # Las propuestas pendientes que ya no se sostienen se retiran de la lista (no se guardan como rechazadas:
    # simplemente dejan de proponerse, y volverán a aparecer si la clasificación vuelve a justificarlas).
    for p in db.scalars(select(Proposal).where(Proposal.type == "RETIRAR_ETIQUETA", Proposal.status == "propuesta")):
        nombre = (p.payload or {}).get("tag")
        t = retirables.get(nombre)
        if not t:
            db.delete(p)
        elif p.payload.get("clase") != t.classification:
            p.title = f"Retirar etiqueta «{t.name}» ({t.classification})"
            p.detail = (t.reason or "") + ". Al aprobar, se marca como retirada en la app y se anota en el historial; el borrado en ManyChat lo haces tú con un clic desde ManyChat (la app no borra allí)."
            p.payload = {**p.payload, "clase": t.classification}
    db.flush()
    for t in retirables.values():
